...
def _upload_file_entry(
    ...
    if file_path is None or not file_path.exists():
        ...
        return True
    try:
        result = drive.upload_file(file_path, folder_id, mime_type, file_name=file_name)
...
def cmd_watch(args: argparse.Namespace) -> int:
    ...
    while True:
        try:
            ...
            for job in failed:
                retry_count = int(job.get("error", "").count("auto-retry") )
                if retry_count < 3:
                    fid = job.get("file_id")
                    if fid:
                        error_msg = job.get("error", "")
                        new_error = error_msg + f" [auto-retry {retry_count+1}/3]"
                        sheets.update_job_status(spreadsheet_id, queue_tab, fid, "pending", error=new_error)
                        print(f"  Auto-retrying: {job.get('file_name')} (attempt {retry_count+1}/3)")
        except Exception as e:
        ...
        for job in pending:
            ...
            # Allow deletion if file is in INPUT or any subfolder of INPUT
            def is_in_input_folder(folder_id, root_id):
                ...
                try:
                    ...
                    for parent in parents:
                        if is_in_input_folder(parent, root_id):
                            return True
                except Exception:
                    pass
                return False

            if recorded_folder and not is_in_input_folder(recorded_folder, input_folder_id):
                print(
                    f"  WARNING: 跳過刪除 {fname} — "
                    f"父資料夾 {recorded_folder} 不在 INPUT 底下 ({input_folder_id})",
                    file=sys.stderr,
                )
            else:
                for _attempt in range(3):
                    try:
                        ...
                        if _attempt < 2:
                            print(f"  WARNING: 刪除失敗，重試中... ({_attempt+1}/3)", file=sys.stderr)
                            time.sleep(5)
                        else:
...
def cmd_upload(args: argparse.Namespace) -> int:
    ...
    # --- Create Drive output folder (one level: OUTPUT/<case_no>/) ---
    try:
        case_folder = drive.find_or_create_folder(case_no, output_folder_id)
        manifest["output_folder_id"] = case_folder
        drive_manifest.save(manifest, work_dir)
    except Exception as e:
    ...
    any_failed |= _upload_file_entry(
        ...
        "application/pdf", case_folder, force,
        "No *_bookmarked.pdf found in work directory",
    )

    # --- 2. Upload OCR markdown (renamed to <pdf_stem>_ocr.md) ---
    ...
    any_failed |= _upload_file_entry(
        ...
        ocr_path,
        "text/markdown", case_folder, force,
        "No merged_ocr.md or **/ocr_corrected.md found",
        file_name=f"{pdf_stem}_ocr.md",
    )
